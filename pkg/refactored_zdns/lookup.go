...
func (r *Resolver) iterateOnAuthorities(q Question, depth int, result Result, layer string, trace []interface{}) (Result, []interface{}, Status, error) {
	...
	for i, elem := range result.Authorities {
		...
		ns, ns_status, layer, trace := r.extractAuthority(elem, layer, depth, result, trace)
		...
		iterateResult, trace, status, err := r.iterativeLookup(q, ns, depth+1, layer, trace)
...
func (r *Resolver) extractAuthority(authority interface{}, layer string, depth int, result Result, trace []interface{}) (string, Status, string, []interface{}) {
...
func (r *Resolver) iterativeLookup(q Question, nameServer string,
	...
	if status != STATUS_NOERROR {
		...
		return r.iterateOnAuthorities(q, depth, result, layer, trace)
	} else {
