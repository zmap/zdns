...
def run_zdns(flags, executable=ZDNS_EXECUTABLE):
    input_domains = [
        domain for domain in expected_test_data_lookup_domain_to_IPs.keys()
    ]
...
def verify_output(stdout, module):
    ...
    for line in output_lines:
        ...
        assert (
            actual_domain in expected_test_data_lookup_domain_to_IPs
        ), f"Domain not found in expected data: {actual_domain}"
        ...
        assert actual_IPs == set(
            expected_test_data_lookup_domain_to_IPs[actual_domain]
        ), (
