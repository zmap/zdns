...
var nsRecords = make(map[string]*core.NSResult)
var nsStatus = core.STATUS_NOERROR

// Mock the actual NS lookup.
func mockNSLookup(r *core.Resolver, lookupName, nameServer string) (interface{}, core.Trace, core.Status, error) {
	if res, ok := nsRecords[lookupName]; ok {
		return res, nil, nsStatus, nil
	} else {
		return core.NSResult{}, nil, core.STATUS_NXDOMAIN, nil
	}
}

func InitTest() (*AxfrLookupModule, *core.Resolver) {
	axfrRecords = make(map[string][]dns.RR)
	transferError = ""
	envelopeError = ""

	nsRecords = make(map[string]*core.NSResult)
	nsStatus = core.STATUS_NOERROR

	cc := new(cli.CLIConf)
	cc.NameServers = []string{"127.0.0.1"}

	rc := new(core.ResolverConfig)
...
func TestLookupSingleNS(t *testing.T) {
	...
	nsRecords["example.com"] = &core.NSResult{
		Servers: []core.NSRecord{
			{
				Name:          ns1 + ".",
				Type:          "NS",
	...
	for i, rec := range axfrRecords[hostPort1] {
		expectedServersMap[ip1][i] = core.ParseAnswer(rec)
	}

	res, _, status, _ := axfrMod.Lookup(resolver, "example.com", "")
	assert.Equal(t, status, core.STATUS_NOERROR)
...
func TestLookupTwoNS(t *testing.T) {
	...
	for i, rec := range axfrRecords[hostPort2] {
		expectedServersMap[ip2][i] = core.ParseAnswer(rec)
	}
...
func TestFailureInTransfer(t *testing.T) {
	...
	assert.Equal(t, res.(AXFRResult).Servers[0].Status, core.STATUS_ERROR)
...
func TestNXDomain(t *testing.T) {
	axfrMod, resolver := InitTest()
	res, _, status, _ := axfrMod.Lookup(resolver, "example.com", "")
	assert.Equal(t, status, core.STATUS_NXDOMAIN)
