...
func (r *Resolver) lookup(ctx context.Context, qWithMeta *QuestionWithMetadata, nameServers []NameServer, isIterative bool, trace Trace, depth int) (*SingleQueryResult, Trace, Status, error) {
	...
	if isIterative {
		...
		res, isCached, status, err, trace = r.cyclingLookup(ctx, qWithMeta, nameServers, qWithMeta.Q.Name, depth, true, trace)
...
func (r *Resolver) iterativeLookup(ctx context.Context, qWithMeta *QuestionWithMetadata, nameServers []NameServer,
	...
	defer cancel()
	result, isCached, status, err, trace := r.cyclingLookup(iterationStepCtx, qWithMeta, nameServers, layer, depth, false, trace)
...
func (r *Resolver) cyclingLookup(ctx context.Context, qWithMeta *QuestionWithMetadata, nameServers []NameServer, layer string, depth int, recursionDesired bool, trace Trace) (*SingleQueryResult, IsCached, Status, error, Trace) {
	...
	for *qWithMeta.RetriesRemaining >= 0 {
		if util.HasCtxExpired(&ctx) {
			return &SingleQueryResult{}, false, StatusTimeout, nil, trace
		}
		...
		// perform the lookup
		result, isCached, status, err, trace = r.cachedLookup(ctx, qWithMeta.Q, nameServer, layer, depth, recursionDesired, cacheBasedOnNameServer, cacheNonAuthoritative, trace)
		if status == StatusNoError {
			r.verboseLog(depth+1, "Cycling lookup successful. Name: ", qWithMeta.Q.Name, ", Layer: ", layer, ", Nameserver: ", nameServer)
			return result, isCached, status, err, trace
			...
			return result, isCached, status, errors.New("cycling lookup failed - out of retries"), trace
		}
	...
	return &SingleQueryResult{}, false, StatusError, errors.New("cycling lookup function did not exit properly"), trace
}
...
func (r *Resolver) cachedLookup(ctx context.Context, q Question, nameServer *NameServer, layer string, depth int, requestIteration, cacheBasedOnNameServer, cacheNonAuthoritative bool, trace Trace) (*SingleQueryResult, IsCached, Status, error, Trace) {
	var isCached IsCached
	isCached = false
	...
	if isValid, reason := nameServer.IsValid(); !isValid {
		return &SingleQueryResult{}, false, StatusIllegalInput, fmt.Errorf("invalid nameserver (%s): %s", nameServer.String(), reason), trace
	}
	...
	if ok {
		...
		return cachedResult, isCached, StatusNoError, nil, trace
	}
	...
	if r.blacklist != nil {
		if blacklisted, isBlacklistedErr := r.blacklist.IsBlacklisted(nameServer.IP.String()); isBlacklistedErr != nil {
			return nil, isCached, StatusError, errors.Wrapf(isBlacklistedErr, "could not check blacklist for nameserver IP: %s", nameServer.IP.String()), trace
		} else if blacklisted {
			return &SingleQueryResult{}, isCached, StatusBlacklist, nil, trace
		}
	}
	var authName string
	if !requestIteration {
		...
		if err != nil {
			...
			return &SingleQueryResult{}, isCached, StatusAuthFail, errors.Wrap(err, "could not get next authority with name: "+name+" and layer: "+layer), trace
		}
		if name != layer && authName != layer {
			...
			if authName == "" {
				...
				return &SingleQueryResult{}, isCached, StatusAuthFail, nil, trace
			}
			...
			if ok {
				...
				if len(cachedResult.Additional) > 0 && len(cachedResult.Authorities) > 0 {
					return cachedResult, true, StatusNoError, nil, trace
				}
	...
	if err != nil {
		return &SingleQueryResult{}, false, StatusError, fmt.Errorf("could not get a connection info to query nameserver %s: %v", nameServer, err), trace
	}
	// check that our connection info is valid
	if connInfo == nil {
		return &SingleQueryResult{}, false, StatusError, fmt.Errorf("no connection info for nameserver: %s", nameServer), trace
	}
	var result *SingleQueryResult
	var rawResp *dns.Msg
	var status Status
	if r.dnsOverHTTPSEnabled {
		...
		return &SingleQueryResult{}, false, StatusError, errors.New("no connection info for nameserver"), trace
	}

	if err != nil {
		return &SingleQueryResult{}, isCached, status, errors.Wrap(err, "could not perform lookup"), trace
	}
	r.verboseLog(depth+2, "Results from wire for name: ", q, ", Layer: ", layer, ", Nameserver: ", nameServer, " status: ", status, " , err: ", err, " result: ", result)

	if status == StatusNoError && result != nil {
		if r.dnsSecEnabled {
			validated, err, trace := r.validateChainOfDNSSECTrust(ctx, rawResp, q, nameServer, !requestIteration, depth+2, trace)
			r.verboseLog(depth+2, "DNSSEC validation result: ", validated, " err: ", err)
			if err != nil {
				return result, isCached, StatusError, errors.Wrap(err, "could not validate chain of DNSSEC trust"), trace
			}
		}
...
func (r *Resolver) validateChainOfDNSSECTrust(ctx context.Context, msg *dns.Msg, q Question, nameServer *NameServer, isIterative bool, depth int, trace Trace) (bool, error, Trace) {
	...
	if len(typeToRRSigs) == 0 {
		...
		return false, nil, trace
	}

	if len(typeToRRSets) != len(typeToRRSigs) {
		return false, errors.New("mismatched number of RRsets and RRSIGs"), trace
	}

	// Verify if for each RRset there is a corresponding RRSIG
	for rrType := range typeToRRSets {
		if _, ok := typeToRRSigs[rrType]; !ok {
			return false, fmt.Errorf("found RRset for type %s but no RRSIG", dns.TypeToString[rrType]), trace
		}
	}
	...
	if q.Type == dns.TypeDNSKEY {
		...
		for _, rr := range typeToRRSets[dns.TypeDNSKEY] {
			...
			if dnskey.Flags == keySigningKeyFlag {
				...
				return false, fmt.Errorf("unknown DNSKEY flag: %d", dnskey.Flags), trace
			}
		}

		if ksk == nil {
			return false, errors.New("could not find KSK"), trace
		}
		...
		if err := typeToRRSigs[dns.TypeDNSKEY][0].Verify(ksk, typeToRRSets[dns.TypeDNSKEY]); err != nil {
			return false, errors.Wrap(err, "could not verify DNSKEY RRSIG"), trace
		}
	...
	if status != StatusNoError || err != nil {
		return false, fmt.Errorf("cannot get DNSKEYs, status: %s, err: %v", status, err), trace
	}
	...
	for rrType, rrSet := range typeToRRSets {
		...
		if err := sig.Verify(zsk, rrSet); err != nil {
			return false, errors.Wrapf(err, "could not verify RRSIG for type %s", dns.TypeToString[rrType]), trace
		}
	}
